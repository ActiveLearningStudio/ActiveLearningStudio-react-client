...
const Media = () => {
  ...
  const mediaLibrary = (sourceName) =>
    updateLibrary?.filter((data) => data.name === sourceName)[0]?.pivot
      ?.h5p_library;
  const onUpdateMediaSources = async () => {
    ...
    orgVideoSource?.map((videoSource) => {
      media_ids.push({
        ...
        lti_tool_settings_status: videoSource.pivot.lti_tool_settings_status
          ? 1
          : 0,
        media_sources_show_status: videoSource.pivot.media_sources_show_status
          ? 1
          : 0,
      });
    });
                          ...
                          <div>
                            <input
                              ...
                              checked={
                                orgVideoSource?.filter(
                                  (source) =>
                                    source.pivot.media_sources_show_status ===
                                    true
                                )?.length === 6
                              }
                              onChange={(e) => {
                              ...
                              return (
                                <div className="media-version-options">
                                  ...
                                  >
                                    ...
                                    <span
                                      id={
                                        isVideoSource?.id && "span-sub-selected"
                                      }
                                      className="span-sub"
                                    >
                                      {source.name}
                              ...
                              if (e.target.checked) {
                                setorgImageSource(
                                  allImageSource.filter(
                                    (source) => source.name !== "Safari Montage"
                                  )
                                );
                              } else {
                                setorgImageSource([]);
