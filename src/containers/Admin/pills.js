...
export default function Pills(props) {
  ...
  const { activityTypes, activityItems, usersReport, allbrightCove, teams, indActivities } = useSelector((state) => state.admin);
        ...
        ?.map((asset) => (
          <Tab key={asset} eventKey={asset} title={asset}>
            <div key={asset} className="module-content-inner">
              ...
              {/* Ind. Activity Start */}
              {type === 'IndActivities' && subTypeState === 'All independent activities' && (
                <Starter
                  paginationCounter={true}
                  size={size}
                  setSize={setSize}
