...
import { adminIntActivities } from "store/actions/indActivities";
import {
  getJobListing,
  getLogsListing,
  getLtiTools,
...
export default function Pills(props) {
  ...
  useEffect(() => {
    if (type === "Stats" && subTypeState === "Report" && searchQueryStats) {
      ...
      let result = dispatch(
        getUserReport("all", size, activePage, searchQueryStats)
      );
      result.then((data) => {
      ...
      type === "Stats" &&
      subTypeState === "Report" &&
      (activePage !== organization?.activePage || size !== organization?.size)
    ) {
      //pagination
      setUserReportStats(null);
    ...
    if (
      ...
      let result = dispatch(
        getJobListing(jobType.value, size, activePage, searchQueryStats)
      );
    ...
    if (
      ...
      let result = dispatch(
        getLogsListing(logType.value, size, activePage, searchQueryStats)
      );
  ...
  const searchUserReportQueryHandler = async (query, subTypeRecieved) => {
    if (subTypeRecieved === "Report") {
      if (query) {
        ...
        let result = await dispatch(
          getUserReport("all", size, undefined, query)
        );
    ...
    if (subTypeRecieved === "Queues: Jobs") {
      if (query) {
        let result = dispatch(
          getJobListing(jobType.value, size, undefined, query)
        );
        result.then((data) => {
          setJobs(data.data);
    ...
    if (subTypeRecieved === "Queues: Logs") {
      if (query) {
        let result = dispatch(
          getLogsListing(logType.value, size, undefined, query)
        );
        result.then((data) => {
          setLogs(data.data);
          if (data?.data?.length > 0) {
  ...
  const searchQueryChangeHandlerActivityTypes = (search) => {
    ...
    dispatch(
      getActivityTypes(activeOrganization?.id, 1, size, "", "", encodeQuery)
    );
  };
  ...
  const searchQueryChangeHandlerActivityItems = (search) => {
    ...
    dispatch(
      getActivityItems(activeOrganization?.id, encodeQuery, activePage, size)
    );
  };
  ...
  const searchQueryChangeHandlerLtiTool = (search) => {
    ...
    const encodeQuery = encodeURI(search.target.value);
    setSearchQuery(encodeQuery);
    const result = adminService.getLtiTools(
      activeOrganization?.id,
      1,
      size,
      encodeQuery,
  ...
  // Filter For Ind Activity
  const filterIndActivitySearch = useCallback(() => {
    if (libraryReqSelected) {
      // const libraryrequest = adminService.getAllProjectIndex(
      //   activeOrganization?.id,
      //   activePage,
      ...
      //   projectFilterObj.shared
      // );
      const libraryrequest = indActivity.allAdminIntActivities(
        activeOrganization?.id,
        activePage,
        1,
        size,
      ...
      const allproject = indActivity.allAdminIntActivities(
        activeOrganization?.id,
        activePage,
        size,
        projectFilterObj.author_id || null,
        ...
        ?.map((asset) => (
          <Tab key={asset} eventKey={asset} title={asset}>
            <div key={asset} className="module-content-inner">
              ...
              {type === "IndActivities" &&
                subTypeState === "All independent activities" && (
                  <Starter
                    ...
                    projectFilterObj={projectFilterObj}
                    setProjectFilterObj={setProjectFilterObj}
                    filterSearch={filterIndActivitySearch}
                    libraryReqSelected={libraryReqSelected}
                    setLibraryReqSelected={setLibraryReqSelected}
                    setCurrentTab={setCurrentTab}
                    setAllProjectTab={setAllProjectTab}
              ...
              {type === "Settings" && subTypeState === "LMS settings" && (
                <Starter
                  type={type}
                  subType={"LMS settings"}
                  subTypeState={subTypeState}
