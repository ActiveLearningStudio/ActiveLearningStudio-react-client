...
import { loadH5pResourceSettingsIndependentActivity } from 'store/actions/gapi';
...
const IndependentActivity = (props) => {
  const { match, h5pSettings, ltiFinished, attemptId, loadH5PSettingsIndependentActivity, passCourseDetails, sendStatement, gradePassBack, activityInit, sendScreenshot } = props;
  const { independentActivityId } = match.params;
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    loadH5PSettingsIndependentActivity(match.params.independentActivityId, studentId, submissionId);
    passCourseDetails({
      courseId,
      issuerClient,
      customApiDomainUrl,
...
IndependentActivity.defaultProps = {
  h5pSettings: null,
  attemptId: null,
};

IndependentActivity.propTypes = {
  match: PropTypes.object.isRequired,
  h5pSettings: PropTypes.object,
  ltiFinished: PropTypes.bool.isRequired,
  attemptId: PropTypes.number,
  loadH5PSettingsIndependentActivity: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  loadH5PSettingsIndependentActivity: (independentActivityId, studentId, submissionId) => dispatch(loadH5pResourceSettingsIndependentActivity(independentActivityId, studentId, submissionId)),
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(IndependentActivity));
