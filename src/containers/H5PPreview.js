...
const H5PPreview = (props) => {
  ...
  const resourceLoaded = async (data) => {
    ...
    const newStyles = data.h5p.settings.core.styles.concat(
  ...
  useEffect(() => {
    if (resourceId !== activityId) {
      ...
      const loadResource = async () => {
        try {
          if (showLtiPreview) {
            const response = await loadH5pResourceSettingsOpen(
              activityId
            );
            if (response.h5p_activity) {
              window.brightcoveAccountId =
                response.h5p_activity?.brightcoveData?.accountId;
              await resourceLoaded(response.h5p_activity);
            }
          } else if (showActivityPreview) {
            const response = await loadH5pResourceSettingsShared(
              activityId
            );
            ...
            if (activities) {
              const response = await indServices.renderh5pIndependent(
                activeOrganization.id,
                activityId
              );
              if (
                response?.["independent-activity"]?.brightcoveData
              ) {
                window.brightcoveAccountId =
                  response.activity?.brightcoveData?.accountId;
              }
              if (response?.["independent-activity"]) {
                await resourceLoaded(
                  response["independent-activity"]
                );
              }
            } else {
  ...
  useEffect(() => {
    ...
    if (
      activityState.h5pObject.externalDispatcher &&
...
const mapDispatchToProps = (dispatch) => ({
  loadH5pResourceProp: (activityId) =>
    dispatch(loadH5pResource(activityId)),
});
