            ...
            <div>
              {!toggleLeft ? (
                <div className="member-name-mark">
                  <span>{`${element?.first_name ? element?.first_name[0] : ""}${
                ...
                <div className="hover-memeber">
                  ...
                  <div className="member-name-mark-hover">
                    ...
                    }${element?.last_name ? element?.last_name[0] : ""}`}</span>
                  </div>
                </div>
              )}
            </div>
              ...
              {(teamPermission?.Team?.includes("team:remove-team-user") ||
                ...
                <Dropdown>
                  <Dropdown.Toggle id="dropdown-basic">
                    {element?.role?.name ||
                      roles?.filter((role) => role.id == element.role_id)[0]
                        ?.name}
                  ...
                  <Dropdown.Menu>
                    {roles?.map((role) => (
                      <Dropdown.Item
                        onClick={() => {
                          if (element.role_id) {
                            const newRoleArray = arrayToRender?.filter(
                              (value) => {
