...
const ModalImage = (props) => {
  ...
  useEffect(() => {
    if (mediaSources.map((obj) => obj.name).includes("Pexels")) {
      ...
      mediaSources.map((obj) => obj.name).includes("Smithsonian")
    ) {
      setactiveKey("Smithsonian");
    } else if (
      mediaSources.map((obj) => obj.name).includes("My device")
    ) {
    ...
    <>
      ...
      >
        <Modal.Body className="thumbnails-modal-body">
          ...
          {activeKey !== "My device" ? (
            <p className="thumbnails-text">
              You are currently viewing images form {activeKey}{" "}
              library.
            </p>
          ) : (
          ...
          >
            {mediaSources.some(
              (obj) =>
                obj.name === "Pexels" && obj.media_type === "Image"
            ) && (
            ...
            {mediaSources.some(
              (obj) =>
                obj.name === "My device" && obj.media_type === "Image"
            ) && <Tab eventKey="My device" title="My Device"></Tab>}
          </Tabs>
        </Modal.Body>
      </Modal>
