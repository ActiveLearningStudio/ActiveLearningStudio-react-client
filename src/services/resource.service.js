...
const loadResource = (resourceId) =>
  httpService
    .get(`/${apiVersion}/load-resource/${resourceId}`)
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));

const uploadResourceThumb = (formData, configData) =>
  httpService
...
const h5pToken = (data) =>
  httpService
    .post(`/${apiVersion}/h5p`, data)
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));

const createActivity = (data) =>
  httpService
    .post(`/${apiVersion}/activities`, data)
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));
