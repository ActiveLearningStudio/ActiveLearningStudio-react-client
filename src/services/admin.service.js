...
const getLtiTools = (subOrgId, page, size, query, column, orderBy, filterBy) =>
  httpService
    .get(
...
const ltiToolType = (subOrgId) =>
  httpService
    .get(`/${apiVersion}/suborganizations/${subOrgId}/lti-tool-type`)
    .then(({ data }) => data)
    .catch((err) => {
      Promise.reject(err);
    });
...
const removeUser = (subOrgId, userId, preserve) =>
  httpService
    .remove(`${apiVersion}/suborganizations/${subOrgId}/remove-user`, {
      user_id: userId,
