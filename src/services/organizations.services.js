...
const getOrganizationSearch = (id, search) =>
  httpService
    .get(
      `/${apiVersion}/suborganizations/${id}/index?query=${search.replace(
        /#/,
        "%23"
      )}`
    )
...
const inviteUserOutside = (id, info) =>
  httpService
...
const getSubOrganizationList = (id, size, page) =>
  httpService
    .get(
      `/${apiVersion}/suborganizations/${id}/index?size=${size}&page=${page}`
    )
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));

const getAllUsers = (id, name, method) =>
  httpService
    .get(
      `/${apiVersion}/suborganizations/${id}/member-options?query=${name}&page=${method}`
    )
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));
...
const updateOrganization = (organizationData, id) =>
  httpService
...
const allPermission = (id) =>
  httpService
    .get(`/${apiVersion}/suborganizations/${id}/permissions`)
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));

const getOrgUsers = (id, page, activeRole, size) =>
  httpService
    .get(
      `/${apiVersion}/suborganizations/${id}/users?page=${page}${
...
const deleteUserFromOrganization = (id, body) =>
  httpService
...
const removeUserFromOrganization = (subOrgId, body) =>
  httpService
...
const searchUserInOrganization = (id, query, page, role) =>
  httpService
    .get(
      `/${apiVersion}/suborganizations/${id}/users?query=${query}${
        page ? `&page=${page}` : ""
      }${role ? `&role=${role}` : ""}`
    )
    .then(({ data }) => data)
...
const getAllRoles = (id) =>
  httpService
    .get(`/${apiVersion}/suborganizations/${id}/roles`)
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));

const addRole = (id, rolesData) =>
  httpService
...
const roleDetail = (id, roleId) =>
  httpService
    .get(`/${apiVersion}/suborganizations/${id}/role/${roleId}`)
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));

const searchOrganization = (searchKeyword) =>
  httpService
    .get(
      `/${apiVersion}/organizations/search?query=${searchKeyword.replace(
        /#/,
        "%23"
      )}`
    )
...
const getOrgsForDeepLinking = (userEmail, ltiClientId) =>
  httpService
    .get(
      `/${apiVersion}/go/lms/organizations?userEmail=${userEmail}&ltiClientId=${ltiClientId}`
    )
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));
