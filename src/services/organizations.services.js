...
const getSubOrganizationList = (id, size, page, query, column = '', orderBy = '') =>
  httpService
    .get(
...
const getOrgUsers = (id, page, activeRole, size, query, column = '', orderBy = '') =>
  httpService
    .get(
      `/${apiVersion}/suborganizations/${id}/users?page=${page}${activeRole ? `&role=${activeRole}` : ''}${size ? `&size=${size}` : ''}
  ${query !== '' ? `&query=${query}` : ''}${column !== '' ? `&order_by_column=${column}` : ''}${orderBy !== '' ? `&order_by_type=${orderBy}` : ''}`,
    )
    .then(({ data }) => data)
    .catch((err) => Promise.reject(err.response.data));
...
const searchUserInOrganization = (id, query, page, role, size, column = '', orderBy = '') =>
  httpService
    .get(
      `/${apiVersion}/suborganizations/${id}/users?query=${query}${page ? `&page=${page}` : ''}${role ? `&role=${role}` : ''}
...
const addRole = (id, rolesData) =>
  httpService
    .post(`/${apiVersion}/suborganizations/${id}/add-role-ui-permissions`, rolesData)
    .then(({ data }) => data)
    .catch((err) => {
      errorCatcher(err.response.data);
      return Promise.reject(err.response.data);
